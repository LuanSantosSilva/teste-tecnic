$(document).ready(function () {
        ...
        let novoProduto = `
            <div class="produto-item flex items-center gap-2">
                <select name="produtos[]" class="produto-select w-full p-2 border rounded-md" required>
                    ${$("#produtos-container select").first().html()}
                </select>
    ...
    $("#add-parcela").click(function () {
        ...
        let novaParcela = `
            <div class="parcela-item">
                ...
                <input type="number" name="parcelas[valor][]" min="0.01" step="0.01" required placeholder="Valor da parcela">
                <input type="date" name="parcelas[vencimento][]" value="${dataVencimento}" required>
    ...
    $(document).on("input", ".quantidade, select[name='produtos[]']", function () {
        calcularTotalVenda();
    });
    ...
    function recalcularNumeracaoParcelas() {
        $("#parcelas-container .parcela-item").each(function (index) {
            $(this).find("input[name='parcelas[numero][]']").val(index + 1);
